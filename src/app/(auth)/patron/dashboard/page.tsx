    ...
    return (
        ...
        >
            ...
            <motion.div variants={itemVariants} className="relative overflow-hidden rounded-[2.5rem] bg-indigo-950 p-8 lg:p-12 shadow-2xl">
                <div className="absolute top-0 right-0 -mt-20 -mr-20 rounded-full w-80 h-80 bg-indigo-500/20 blur-3xl animate-pulse" />
                <div className="absolute bottom-0 left-0 -mb-20 -ml-20 rounded-full w-80 h-80 bg-purple-500/20 blur-3xl animate-pulse" />
                ...
                <div className="relative z-10 flex flex-col justify-between gap-8 lg:flex-row lg:items-center">
                    <div className="max-w-2xl">
                        ...
                        <p className="max-w-lg mt-4 text-lg font-medium leading-relaxed text-white/50">
                    ...
                    <Card className="border-none bg-white/5 backdrop-blur-xl p-8 rounded-[2rem] border border-white/10 w-full lg:w-96 shadow-2xl">
                        <div className="flex flex-col items-center text-center">
                            ...
                            <div className="flex items-center w-full gap-4 p-2 pl-6 border bg-white/10 rounded-2xl border-white/10">
                                <span className="flex-1 text-xl font-black tracking-tighter text-left text-white truncate">{user?.username}</span>
                                <Button 
                                    onClick={copyReferralLink}
                                    className="w-12 h-12 p-0 text-white transition-transform bg-indigo-600 shadow-lg hover:bg-indigo-700 rounded-xl shadow-indigo-500/20 active:scale-95"
                                >
                                    <Copy size={20} />
                                </Button>
                            </div>
            ...
            <motion.div variants={itemVariants} className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
                {stats.map((stat, i) => (
                    <Card key={i} className="relative p-1 overflow-hidden transition-all duration-500 bg-white border-none shadow-sm group rounded-3xl hover:shadow-2xl">
                ...
                {patronData?.patronGroup?.type === 'group' && (
                    <motion.div variants={itemVariants} className="space-y-6">
                        <div className="px-2">
                            ...
                            <p className="mt-1 text-sm font-medium text-zinc-400">Recent organization transactions</p>
                        </div>
                        ...
                        <Card className="border-none bg-white rounded-[2rem] shadow-sm border border-zinc-100 h-full overflow-hidden flex flex-col">
                            <CardContent className="p-0">
                                {(patronData?.transactions || []).length === 0 ? (
                                    <div className="flex flex-col items-center justify-center p-20 text-center">
                                        ...
                                        <p className="text-sm font-bold tracking-widest uppercase text-zinc-400">No activity yet</p>
                                    </div>
                                ) : (
                                    <div className="divide-y divide-zinc-50">
                                        {(patronData?.transactions || []).slice(0, 6).map((tx) => (
                                            <div key={tx.id} className="p-6 transition-colors cursor-pointer hover:bg-zinc-50/50 group">
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        <div className="flex items-center justify-between gap-2">
                                                            <h4 className="text-xs font-black tracking-widest uppercase truncate text-zinc-900">
                            ...
                            {patronData && (patronData.transactions || []).length > 6 && (
                                <div className="p-4 mt-auto border-t bg-zinc-50/50 border-zinc-100">
