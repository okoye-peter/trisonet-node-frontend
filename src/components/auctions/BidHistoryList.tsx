...
export function BidHistoryList({ bids, currentUserId }: { bids: AuctionBid[]; currentUserId?: string }) {
    ...
    if (bids.length === 0) {
        return <div className="px-5 py-10 text-center text-sm text-zinc-400">No bids yet — be the first to bid.</div>;
    }
        ...
        <div className="flex flex-col">
            {visibleBids.map((bid, index) => {
                ...
                return (
                    <div
                        key={bid.id}
                        className={`flex items-center gap-3.5 border-b border-zinc-100 px-5 py-3.5 last:border-none ${isWinner ? 'bg-amber-50' : bid.status === 'pending' && index === 0 ? 'bg-indigo-50' : ''}`}
                    >
                        <div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-xs font-black ${isWinner ? 'bg-amber-100 text-amber-700' : RANK_STYLES[index] || 'bg-zinc-100 text-zinc-400'}`}>
                        ...
                        <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-1.5 truncate text-sm font-semibold text-zinc-900">
                                ...
                                {bid.bidderId === currentUserId && (
                                    <span className="rounded bg-zinc-100 px-1.5 py-0.5 text-[10px] font-bold text-zinc-500">You</span>
                                )}
                                {isWinner && (
                        ...
                        <div className="text-base font-black text-indigo-700">{currency}{bid.amount.toLocaleString()}</div>
                    </div>
                );
            })}
        </div>
